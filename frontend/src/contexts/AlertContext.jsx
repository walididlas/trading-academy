...
const _CTX_PIP_SIZE  = { XAUUSD: 0.1, GBPJPY: 0.01, USDJPY: 0.01, NZDJPY: 0.01 }
const _CTX_PIP_VALUE = { XAUUSD: 100, GBPJPY: 6.67, NZDJPY: 6.67 }

function _buildOutcomeEntry(pair, outcome, sig, ts) {
  ...
  try {
    ...
    if (balance > 0 && sig.entry && sig.sl) {
      const ps      = _CTX_PIP_SIZE[pair]  ?? 0.0001
      const pv      = _CTX_PIP_VALUE[pair] ?? 10
  ...
  let grade = 'F'
  { let s = 0; if (kz) s++; if (trendAligned) s++; if (iccValid) s++; if (rr && parseFloat(rr) >= 3) s++
    grade = s === 4 ? 'A' : s === 3 ? 'B' : s === 2 ? 'C' : 'F' }
...
async function _writeOutcomeToJournal(pair, outcome, signal, ts) {
  try {
    const stored = JSON.parse(localStorage.getItem(`ta_outcome_pending_${pair}`) || 'null')
    const sig    = signal ?? stored?.signal ?? {}
    const entry  = _buildOutcomeEntry(pair, outcome, sig, ts)
